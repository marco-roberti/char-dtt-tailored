...
class EDA(Module):
    ...
    def forward(self, x, y=None):
        ...
        for pos in range(1, max_len):
            # Decoder
            att, dec_output, dec_hidden, prob = self.decoder(last_char_index, dec_output, dec_hidden, enc_annotations)
            ...
            if self.training:
                ...
                attentions.append(att.squeeze(2))
        ...
        if self.training:
            ...
            return torch.stack(probabilities, dim=1).squeeze(), torch.stack(attentions), None
